...
float camX = 00, camY = 200, camZ = 100;
...
void draw_indians(int n_indians, float radius) {
    ...
    for (int i = 0; i < n_indians; i++) {
        float x = radius * sin(alpha) - 5.0f;
        float z = radius * cos(alpha) - 5.0f;
        glPushMatrix();
        glTranslatef(x, 0.8, z);
...
void renderScene(void) {
    ...
    float camera_y = 2 + hf(camera_x + iw / 2, camera_z + ih / 2);
...
void processKeys(unsigned char key, int xx, int yy) {
    ...
    switch (key) {
    case 'w': case 'W':
        camera_x += 0.5 * std::sin(camera_angle);
        camera_z += 0.5 * std::cos(camera_angle);
        break;
    case 's': case 'S':
        camera_x += 0.5 * std::sin(180 + camera_angle);
        camera_z += 0.5 * std::cos(180 + camera_angle);
        break;
    case 'a': case 'A':
        camera_x += 0.5 * std::sin(camera_angle + 90);
        camera_z += 0.5 * std::cos(camera_angle + 90);
        break;
    case 'd': case 'D':
        camera_x += 0.5 * std::sin(camera_angle - 90);
        camera_z += 0.5 * std::cos(camera_angle - 90);
