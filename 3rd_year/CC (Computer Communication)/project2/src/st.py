...
class Application:
    ...
    def __init__(self,argv):
        ...
        self.properties = {
            ...
            "file_sdt": "", # path for file that define ST servers
            "obj_file_sdt": None, # object for st file
            "list_sdt": [], # list of ST servers
        ...
        try:
            keys = ["file_config","file_sdt","port"]
            ...
            if not os.path.exists(argv[1]):
                raise Exception("Inserted SDT file path is not valid!")

            self.properties["obj_file_config"] = Parser(self.properties["file_config"],config=True,dict = self.properties)
            self.properties["obj_file_sdt"] = Parser(self.properties["file_sdt"],st=True,dict = self.properties)
            ...
            for val in vals:
                if val is not None:
                    ...
                    for v in val:
                        ...
                        type = v.split(" ")[1]
                        # creates cache entry
                        entry = self.properties["cache"].createEntry(name,type,v,priority,origin,self.properties["starttime"])
                        # update entries
    ...
    def processTCP(self,socket,address):
        ...
        if q_res.response_code == 2:
            for sdt in self.properties["list_sdt"]:
                socket.sendto(request,self.properties[sdt])
        ...
        if not in self.properties["cache"]:
            for sdt in self.properties["list_sdt"]
                socket.sendto(request,self.properties[sdt_address])
                request = socket.recv(self.properties["size"]).decode(self.properties["encoder"])
                
                #if request is a particular message
