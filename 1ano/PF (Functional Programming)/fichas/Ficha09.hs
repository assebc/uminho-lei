...
acumularNumeros l 
    ...
    | otherwise = do v <- randomRIO (1,90)
                     print v
                     getChar
                     let nl = if elem v l then l else v:l in acumularNumeros nl
...
valida :: Aposta -> Bool
valida (Ap (a:b:c:d:e:[]) (f,g)) = and [elem x [1..50] | x <- [a,b,c,d,e]] && all (\x -> elem x [1..12]) [f,g]
valida _ = False
...
comuns (Ap a1@(a:b:c:d:e:[]) (f,g)) (Ap a2@(h:i:j:k:l:[]) (m,n)) = (contaNums,contaEstr)
    where 
        contaNums = length [x | x <- a1, elem x a2]
        contaEstr = length [y | y <- [f,g], elem y [m,n]]
